...
\author[M.Sc. Y. Höll]{M.Sc. Yannik Höll}
...
\onslide<6-> {
    \begin{itemize}
        \item Spiele wurden entwickelt, um Theorie aufzubauen
        \item Mit entwickelter Theorie ist Untersuchung komplexer Spiele möglich 
    \end{itemize}
}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \only<3-3>{\item Position mit mit 2 Türmen mit gleicher Anzahl an Münzen($N_{k} + N_{k}$): \\ \textbf{Hypothese:} 2. Spieler gewinnt immer, indem er den Gegner nachahmt }
        \item<4-> Position mit mit 2 Türmen mit gleicher Anzahl an Münzen: \\ 2. Spieler gewinnt immer, indem er den Gegner nachahmt
        \item<5-> Position mit mit 2 Türmen mit unterschiedlicher Anzahl an Münzen ($N_{n} + N_{k}, n > k$): \\ 1. Spieler gewinnt immer, indem er $N_{n} + N_{k}$ nach $N_{k} + N_{k}$ überführt
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \end{block}
    \onslide<2-> {
        $\forall k,n \in \mathbb{N}\colon n < 2^k \Rightarrow *2^k + *n = *(2^k + n)$ \\
    }
    \onslide<3-> {
        $\forall k,n \in \mathbb{N}\colon k \neq n: *2^k + *2^n = *(2^k + 2^n)$ \\
        $\forall k \in \mathbb{N}\colon *2^k + *2^k = 0$ \\
    }
    \onslide<4-> {
        \vspace*{20px}
...
\begin{frame}
    ...
    \onslide<3->{\begin{equation*}
        \forall k\in \mathbb{N}\colon k_0 2^0 + k_1 2^1 + k_2 2^2 + \ldots = \sum_{i=0}^\infty k_i\cdot 2^i, \: k_i \in \{0,1\}
    \end{equation*}}
\end{frame}
...
\begin{frame}
    ...
    $G = \{G^L_0, G^L_1, \ldots \mid \: \ldots\}$, dann ist $G^L$ beste Option für Links, genau dann wenn $$\forall i\in \mathbb{N}\colon G^L \geq G^L_i.$$
\end{frame}
...
\begin{frame}
    ...
    \begin{block}{Addition von Kombinatorischen Spielen}
        Seien $G = \{G^L| \: G^R\}, H = \{H^L | \: H^R\}$ kombinatorische Spiele, dann
        \[ G+H = \{G^L + H, G + H^L | \: G^R + H, G + H^R\}. \]
    \end{block}

    \begin{block}{Inverse von Kombinatorischen Spielen}
        Sei $G = \{G^L| \: G^R\}$ kombinatorisches Spiel, dann
        \[ -G = \{-G^R |\: -G^L \}.\]
...
\begin{frame}
    ...
    Seien $G$ und $H$ kombinatorische Spiele, dann
...
\begin{frame}
    ...
    Seien $G = \{A,B,C,\cdots | \: D,E,F,\cdots\}$ und $D^L = \{U,V,W\cdots | \: X,Y,Z,\cdots\}$ kombinatorische Spiele, dann \\
    \begin{center}
        $D$ ist umkehrbarer Zug, genau dann wenn $D^L \geq G$.
    \end{center}
    Also $G = \{A,B,C,\cdots | D,E,F,\cdots\} \leq \{A,B,C,\cdots | \: \alert{X,Y,Z,\cdots},E,F\cdots\} = H$
    \end{block}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item<2-> $* \: || \: 0$, "Star confused with 0", Stern ist ununterscheidbar von 0
