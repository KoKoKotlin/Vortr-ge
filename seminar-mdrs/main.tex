...
\begin{defin}
  ...
  Now define the concatenation of paths $*\colon \mathcal{C}_{\theta}(X, x_0) \times \mathcal{C}_{\theta}(X, x_0) \to \mathcal{C}_{\theta}(X, x_0), (x, y) \mapsto x * y$ where
  \begin{equation*}
...
\begin{proof}
  ...
  \textit{Inverse:}
  Let $x \in \mathcal{C}_{\theta}(X, x_0)$ with length $m$. Define the path $x^-\colon [m] \to X, t \mapsto x(m - t)$.

  Define $H\colon [2m] \times [m] \to X$ given by the following schema
  \begin{equation*}
    \begin{matrix}
